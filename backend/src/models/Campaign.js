...
class Campaign {
  ...
  static async create(userEmail, campaignData) {
    try {
      const { name, subject, content, recipients, scheduled_at, template_id } = campaignData;
      ...
      const result = await query(`
        ...
        RETURNING *
      `, [
        userEmail, name, subject, content, 
        JSON.stringify(recipients), scheduled_at, template_id
      ]);
    ...
    try {
      ...
      for (const recipient of recipients) {
        try {
          ...
          `, [campaignId, recipient.email, recipient.name || null, result.messageId]);

          sendResults.push({ email: recipient.email, status: 'sent', messageId: result.messageId });
        } catch (error) {
          // Log failed send
          await query(`
            INSERT INTO campaign_recipients (
          ...
          `, [campaignId, recipient.email, recipient.name || null, error.message]);
      ...
      const finalStatus = failedCount === sendResults.length ? 'failed' : 
                         failedCount > 0 ? 'partially_sent' : 'sent';
  ...
  static async getAnalytics(campaignId, userEmail) {
    try {
      ...
      const openRate = sentCount > 0 ? (parseInt(stats.opened_count) / sentCount * 100).toFixed(2) : 0;
      const clickRate = sentCount > 0 ? (parseInt(stats.clicked_count) / sentCount * 100).toFixed(2) : 0;
  ...
  static async createTemplate(userEmail, templateData) {
    try {
      const { name, content, thumbnail, is_global = false } = templateData;
