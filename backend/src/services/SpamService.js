...
class SpamService {
  ...
  async testSpam(emailContent, sender, recipient) {
    try {
      ...
      const fullMessage = this.createFullEmailMessage(emailContent, sender, recipient);
      ...
      const heuristicResult = await this.testWithHeuristics(emailContent, sender);
      
      // Combine results
      const combinedScore = Math.max(spamResult.score, heuristicResult.score);
      ...
      return {
        ...
        recommendations: this.generateRecommendations(spamResult, heuristicResult)
      };
    } catch (error) {
  ...
  async testWithSpamAssassin(emailMessage) {
    try {
      ...
      const tempFile = path.join(os.tmpdir(), `spam_test_${Date.now()}.eml`);
      ...
      if (scoreLine) {
        ...
        const testLines = lines.filter(line => line.match(/^\s*[\d.-]+\s+\w+/));
  ...
  async testWithHeuristics(content, sender) {
    ...
    const spamKeywords = [
      ...
      'urgent', 'immediate', 'cash', 'profit', 'income',
    ...
    if (exclamationCount > 3) {
      const exclamationScore = Math.min((exclamationCount - 3) * 0.3, 1.5);
    ...
    const textLength = content.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim().length;
...
${content}`;
  ...
  generateRecommendations(spamResult, heuristicResult) {
      ...
      spamResult.tests.forEach(test => {
        if (test.score > 1.0) {
          switch (test.name) {
            case 'BAYES_99':
            case 'BAYES_95':
              recommendations.push('Content matches known spam patterns. Consider revising the message text.');
              break;
            case 'URIBL_BLACK':
              recommendations.push('Contains URLs from blacklisted domains. Remove or replace suspicious links.');
              break;
            case 'RAZOR2_CHECK':
              recommendations.push('Content matches known spam signatures. Rewrite the message content.');
              break;
            case 'DCC_CHECK':
              recommendations.push('Similar content has been sent in bulk. Make the message more unique.');
              break;
          }
        }
      });
      ...
      heuristicResult.tests.forEach(test => {
        switch (test.name) {
          case 'SUSPICIOUS_KEYWORDS':
            recommendations.push('Remove or replace suspicious keywords that trigger spam filters.');
            break;
          case 'EXCESSIVE_CAPS':
            recommendations.push('Reduce the use of capital letters. Use normal sentence case.');
            break;
          case 'EXCESSIVE_EXCLAMATION':
            recommendations.push('Reduce the number of exclamation marks for a more professional tone.');
            break;
          case 'SUSPICIOUS_URLS':
            recommendations.push('Replace shortened URLs with full domain names from reputable sources.');
            break;
          case 'HIGH_HTML_RATIO':
            recommendations.push('Add more text content relative to HTML markup.');
            break;
        }
      });
    }
    ...
    if (spamResult.score > 3.0 || heuristicResult.score > 2.0) {
      recommendations.push('Consider adding a plain text version of your email.');
      recommendations.push('Ensure your sending domain has proper SPF, DKIM, and DMARC records.');
      recommendations.push('Use a consistent sender name and email address.');
      recommendations.push('Include an unsubscribe link for marketing emails.');
    }
  ...
  async testSenderReputation(senderDomain, senderIP) {
    ...
    try {
      ...
      const avgScore = results.checks.reduce((sum, check) => sum + check.score, 0) / results.checks.length;
      results.overallScore = avgScore;
      results.reputation = avgScore >= 7 ? 'good' : avgScore >= 4 ? 'neutral' : 'poor';
  ...
  async checkDomainReputation(domain) {
    ...
    return {
      ...
      details: isSpamDomain ? 'Domain is known for temporary/spam emails' : 'Domain appears clean'
    };
  }
