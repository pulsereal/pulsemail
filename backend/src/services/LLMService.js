...
class LLMService {
    constructor() {
        this.settings = LLMSettingsService.shared();
    }

    /**
     * The endpoint the administrator configured, or null when the feature is
     * switched off. Every caller already handles null by degrading rather than
     * failing, so a disabled or misconfigured endpoint is never fatal.
     */
    async client() {
    ...
    async model() {
        return (await this.settings.get()).model;
    }
    ...
    async generateReply(
        ...
        try {
            ...
            if (!openai) {
                throw new Error(
                    "No LLM endpoint is configured. An administrator can set one up under AI settings."
                );
            }
            ...
            const preferences =
                await this.getUserReplyPreferences(recipientEmail);
...
- Maintain the conversation context`;
        ...
        if (preferences.autoResponses) {
            prompt += `\n- Use these common responses when appropriate: ${JSON.stringify(
                preferences.autoResponses
            )}`;
        }
    ...
    generateSuggestions(analysis) {
        ...
        if (analysis.urgency === "high") {
            suggestions.push(
                "This email appears urgent. Consider reviewing and sending quickly."
            );
        }

        if (analysis.sentiment === "negative") {
            suggestions.push(
                "The original email has negative sentiment. Consider a more empathetic response."
            );
        }

        if (analysis.questions && analysis.questions.length > 0) {
            suggestions.push(
                `Make sure to address these questions: ${analysis.questions.join(
                    ", "
                )}`
            );
        }
        ...
        if (analysis.requiresAction) {
            suggestions.push(
                "This email requires action. Consider adding specific next steps or deadlines."
            );
        }
...
- Maintains a professional but friendly tone`;
            ...
            return {
                subject: this.extractSubjectFromReply(
    ...
    async summarizeEmail(emailContent, maxLength = 150) {
        try {
            const openai = await this.client();
            if (!openai) {
                // Fallback to simple truncation if OpenAI is not configured
