...
class AutomationService {
    ...
    /**
     * One instance per process. Two would each register their own cron jobs and
     * every scheduled rule would fire twice.
     */
    static shared() {
        if (!shared) shared = new AutomationService();
        return shared;
    }

    /**
     * Called once from startServer, after the schema bootstrap has run. Doing
     * this in the constructor would query tables that do not exist yet.
     */
    async init() {
    ...
    async processIncomingEmail(emailData) {
        try {
            ...
            const applicableRules = Array.from(
    ...
    checkEmailConditions(emailData, conditions) {
        try {
            ...
            if (conditions.sender) {
                if (
                    conditions.sender.includes &&
                    !from
                        .toLowerCase()
                        .includes(conditions.sender.includes.toLowerCase())
                ) {
                    return false;
                }
                if (
                    conditions.sender.equals &&
                    from.toLowerCase() !==
                        conditions.sender.equals.toLowerCase()
                ) {
                    return false;
                }
                if (conditions.sender.domain) {
                    ...
                    if (
                        senderDomain !== conditions.sender.domain.toLowerCase()
                    ) {
                        return false;
                    }
                }
            ...
            if (conditions.subject) {
                if (
                    ...
                    !subject
                        .toLowerCase()
                        .includes(conditions.subject.contains.toLowerCase())
                ) {
                    return false;
                }
                if (
                    conditions.subject.matches &&
                    !new RegExp(conditions.subject.matches, "i").test(subject)
                ) {
                    return false;
                }
            }
            ...
            if (conditions.content) {
                if (
                    ...
                    !content
                        .toLowerCase()
                        .includes(conditions.content.contains.toLowerCase())
                ) {
                    return false;
                }
                if (
                    conditions.content.matches &&
                    !new RegExp(conditions.content.matches, "i").test(content)
                ) {
                    return false;
                }
            }
            ...
            if (conditions.time) {
                ...
                if (
                    conditions.time.business_hours_only &&
                    (hour < 9 || hour > 17)
                ) {
                    return false;
                }
                if (
                    ...
                    hour < parseInt(conditions.time.after)
                ) {
                    return false;
                }
                if (
                    conditions.time.before &&
                    hour > parseInt(conditions.time.before)
                ) {
                    return false;
                }
            }
            ...
            for (const action of actions) {
                switch (action.type) {
                    ...
                    case "send_notification":
                        ...
                        break;
                    case "llm_categorize":
                        await this.executeLLMCategorize(
                            rule,
                            emailData,
                            action
                        );
    ...
    async executeAddLabel(rule, emailData, action) {
      ...
      `,
            ...
            console.log(
                `Label "${action.label}" added to email ${emailData.uid}`
            );
        } catch (error) {
    ...
    async executeCreateTask(rule, emailData, action) {
        try {
            const taskTitle = this.replacePlaceholders(
                action.title,
                emailData,
                rule
            );
            const taskDescription = this.replacePlaceholders(
                action.description || "",
                emailData,
                rule
            );
    ...
    async executeSendNotification(rule, emailData, action) {
        try {
            const notificationContent = this.replacePlaceholders(
                action.message,
                emailData,
                rule
            );
        ...
        try {
            ...
            await query(
                `INSERT INTO email_classifications
                   (user_email, message_key, folder, uid, uidvalidity,
                    category, importance, method, message_date)
                 VALUES ($1, $2, $3, $4, 0, $5, $6, 'llm', $7)
                ...
                [
                    rule.user_email,
                    emailData.messageId || `loc:${folder}:0:${emailData.uid}`,
                    folder,
                    emailData.uid,
                    category,
                    category === "important" ? 90 : 30,
                    emailData.date || null,
                ]
            );
        ...
        try {
            ...
            for (const action of rule.actions) {
                switch (action.type) {
                    ...
                    default:
                        console.warn(
                            `Unsupported scheduled action: ${action.type}`
                        );
                }
            }
            ...
            console.error(
                `Failed to execute scheduled rule ${rule.id}:`,
                error
            );
    ...
    async executeSendScheduledEmail(rule, action) {
        ...
        console.log(
            `Scheduled email sent to ${action.recipients.length} recipients`
        );
    }
    ...
    async executeFollowUp(followUp) {
        try {
            const { user_email, original_email_data, follow_up_type, content } =
                followUp;
