...
router.get("/", authenticateToken, async (req, res) => {
    try {
        ...
        let campaigns = await Campaign.getUserCampaigns(
            email,
            parseInt(limit),
            parseInt(offset)
        );
...
router.post("/", authenticateToken, async (req, res) => {
    try {
        ...
        for (const recipient of recipients) {
            if (!recipient.email) {
                return res.status(400).json({
                    error: `Email is required for recipient`,
                });
            }

            // Skip email validation in development mode
            if (
...
router.put("/:id", authenticateToken, async (req, res) => {
    try {
        ...
        if (existingCampaign.status === "sent") {
            return res
                .status(400)
                .json({ error: "Cannot edit sent campaigns" });
        }
...
router.post("/:id/schedule", authenticateToken, async (req, res) => {
    try {
        ...
        if (!scheduled_at) {
            return res
                .status(400)
                .json({ error: "Scheduled date is required" });
        }
...
router.post("/recipients/import", authenticateToken, async (req, res) => {
    try {
        ...
        if (emailIndex === -1) {
            return res
                .status(400)
                .json({ error: "Email column not found in CSV" });
        }

        // Find name column (optional)
        const nameIndex = headers.findIndex(
            (h) => h === "name" || h === "full name" || h === "first name"
        );
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            // Validate email (skip in development mode)
            if (
                process.env.NODE_ENV === "development" &&
                process.env.USE_MOCK_DATA === "true"
            ) {
...
router.get("/performance/summary", authenticateToken, async (req, res) => {
    try {
      ...
      SELECT 
        ...
        COUNT(CASE WHEN created_at >= NOW() - INTERVAL '${parseInt(
            period
        )} days' THEN 1 END) as recent_campaigns
    ...
    `,
        ...
        const deliveryRate =
            recipientStats.total_recipients > 0
                ? (
                      (sentCount / parseInt(recipientStats.total_recipients)) *
                      100
                  ).toFixed(2)
                : 0;
        const openRate =
            sentCount > 0
                ? (
                      (parseInt(recipientStats.opened_count) / sentCount) *
                      100
                  ).toFixed(2)
                : 0;
        const clickRate =
            sentCount > 0
                ? (
                      (parseInt(recipientStats.clicked_count) / sentCount) *
                      100
                  ).toFixed(2)
                : 0;
