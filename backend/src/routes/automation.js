...
router.get("/rules", authenticateToken, async (req, res) => {
    try {
        ...
        let rules = await automationService.getUserRules(
            email,
            parseInt(limit),
            parseInt(offset)
        );
...
router.post("/rules/:id/test", authenticateToken, async (req, res) => {
    try {
        ...
        if (!test_email_data) {
            return res
                .status(400)
                .json({ error: "Test email data is required" });
        }
...
router.post("/follow-ups", authenticateToken, async (req, res) => {
    ...
    `,
            [
                ...
                scheduledDate,
                follow_up_type,
                JSON.stringify(original_email_data || {}),
                use_llm,
                purpose,
            ]
        );
...
router.put("/follow-ups/:id", authenticateToken, async (req, res) => {
    ...
    `,
        ...
        if (current.status === "sent") {
            return res
                .status(400)
                .json({ error: "Cannot edit sent follow-ups" });
        }
...
router.delete("/follow-ups/:id", authenticateToken, async (req, res) => {
    ...
    `,
        ...
        if (result.rows.length === 0) {
            return res
                .status(404)
                .json({ error: "Follow-up not found or cannot be cancelled" });
        }
    ...
    try {
        const templates = [
            ...
            },
            {
                ...
                description:
                    "Forward emails marked as urgent to another address",
                trigger_type: "email_received",
                trigger_conditions: {
            ...
            },
            {
                ...
                description:
                    "Automatically move newsletters to a dedicated folder",
                trigger_type: "email_received",
                trigger_conditions: {
            ...
            },
            {
                ...
                description:
                    "Use AI to automatically categorize all incoming emails",
                trigger_type: "email_received",
                trigger_conditions: {},
                actions: [
                    {
...
router.post("/rules/from-template", authenticateToken, async (req, res) => {
    try {
        ...
        if (!template_id || !name) {
            return res
                .status(400)
                .json({ error: "Template ID and name are required" });
        }
        ...
        const ruleData = {
            ...
            trigger_conditions: {
                ...
                ...customizations.trigger_conditions,
            },
            actions: template.actions.map((action) => ({
                ...action,
                ...customizations.actions?.[0],
            })),
            active: true,
        };
